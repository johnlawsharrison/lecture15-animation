...
//        if(ball.dy < .1) ball.dy = 0;
    ...
    public synchronized void render(Canvas canvas){
        ...
        for(Ball i : touches.values()) {
            canvas.drawCircle(i.cx, i.cy, i.radius, goldPaint); //we can draw directly onto the canvas
        }
    }
    ...
    public synchronized void addTouch(int pointerId, float positionX, float positionY){
        touches.put(pointerId, new Ball(positionX, positionY, 40));
    }
